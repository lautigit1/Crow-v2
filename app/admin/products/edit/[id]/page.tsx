...
import { getProductByIdAdmin, updateProduct } from '../../../../lib/admin-api';
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      try {
        const foundProduct = await getProductByIdAdmin(productId);
        if (foundProduct) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Actualizar producto usando la API del backend
      await updateProduct(product.id, {
        name: formData.nombre || formData.name,
        description: formData.descripcionCompleta || formData.description,
        price: formData.precio || formData.price || 0,
        stock: formData.stock || 0,
        image_url: formData.imagenUrl || formData.image_url,
      });
